...
export default class App extends React.Component {
 ...
 loadPadrinho = async () => {
     const respons = await api.get('/ListaCorBranca'); //buscar dos dados no banco
     this.setState({Cor: respons.data }); // setando o estado de Pu.At. com informações da lista do banco 

     const response = await api.get('/ListaCorParda'); //buscar dos dados no banco
     this.setState({Cor1: response.data }); // setando o estado de Pu.At. com informações da lista do banco 

     const respon = await api.get('/ListaCorPreta'); //buscar dos dados no banco

...
export default class IndexReceita extends Component {
  ...
  componentDidMount() {
    // metodo executa automatico quando inicia a aplicação
    this.loadReceita()
  }
  ...
  render() {
    const { Receita } = this.state // definir variaveis em seu estado atual, carregadas com a lista
    return (
      <div>
        <Data row={filtro(Receita)} />
      </div>
    )
  }
}

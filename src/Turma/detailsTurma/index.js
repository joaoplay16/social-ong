...
class DetailsTurma extends Component {
      ...
      <div>
          ...
          <div className="container">
            <div className="card" style={{ width: "18rem" }}>
              ...
              <ul className="list-group list-group-flush">
                {Object.keys(AlunosDaTurma).length == 0 && (
                  <li className="list-group-item">Lista vazia</li>
                )}
        ...
        <footer>
          <Link
            ...
            to={`/profile/educacao/update-turma/${Turma._id}`}
          >
            Editar
          </Link>
          <Link
            className="btn btn-lg btn-outline-danger"
            to={`/profile/educacao/deletar-turma/${Turma._id}`}
          >
            Deletar
          </Link>
          <Link
            className="btn btn-lg btn-outline-secondary"
            to="/profile/educacao/turmas"
          >
            Voltar
          </Link>
        </footer>
  ...
  async loadTurmaAluno() {
    ...
    if (alunosDaTurma.length > 0) {
      this.setState({
        AlunosDaTurma: alunosDaTurma.reduce(
          (accumulator, current) =>
            Object.assign(
              /*target*/ accumulator,
              /*source*/ {
